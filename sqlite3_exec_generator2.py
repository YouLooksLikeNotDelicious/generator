...
'''.format(struct_file_name.upper())
    for struct_obj in structs:
        code += add_hpp_info(struct_obj, struct_file_name)
    code += '''
...
'''
    ...
    code = ''
    for struct_obj in structs:
        code += add_cpp_info(struct_obj, struct_file_name + '.hpp')
    codes["cpp"] = code
    return codes
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        for struct_file_path in sys.argv[1:]:
            ...
            code = generate_cpp_file_from_code(content, struct_file_name)
